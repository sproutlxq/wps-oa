...
function OnWPSWorkTabLoad(ribbonUI) {
    ...
    wps.PluginStorage.setItem("EnableCopy", true);
    wps.PluginStorage.setItem("EnableFileSaveAsPdfOrXps", true);
    wps.PluginStorage.setItem("EnableFileSaveAsPicture", true);
    return true;
}
...
function OnUploadToServerSuccess(resp) {
    var l_doc = wps.WpsApplication().ActiveDocument;
    console.log(l_doc.Name)

    var isNewName = false;
    var l_showConfirm = wps.PluginStorage.getItem(constStrEnum.Save2OAShowConfirm);

    var l_NofityURL = GetDocParamsValue(l_doc, constStrEnum.notifyUrl);
    var l_docId = GetDocParamsValue(l_doc, constStrEnum.docId);
    var l_attachId = GetDocParamsValue(l_doc, constStrEnum.attachId);
    var l_userName = GetDocParamsValue(l_doc, constStrEnum.userName);

    if(l_doc.Name.indexOf("_") !== -1){
        isNewName = true;
    }

    if(isNewName === false && l_showConfirm) {
        wps.alert("文件上传成功！继续将关闭文档。");
        if (l_doc) {
            console.log("OnUploadToServerSuccess: before Close");
        
            if (l_NofityURL != "") {
                l_NofityURL = l_NofityURL.replace("{?}", "2"); //约定：参数为2则文档被成功上传
                NotifyToServer(l_NofityURL, {"l_docId": l_docId, "l_attachId": l_attachId, "L_userName": l_userName});
            }        
            l_doc.Close(-1); //保存文档后关闭
            console.log("OnUploadToServerSuccess: after Close");
        }
    }
    else if(isNewName === true && l_showConfirm) {
...
function OnAction(control) {
    ...
    switch (eleId) {
        ...
        //     alert("这是OA文档，禁止输出为PDF！");
        //     break;
        // }
        // case "FileSaveAsPicture": {
        //     alert("这是OA文档，禁止输出为图片！");
        //     break;
        // }
...
function OnGetEnabled(control) {
    ...
    switch (eleId) {
        ...
        case "Copy": {
            return wps.PluginStorage.getItem("EnableCopy")
        }
        case "FileSaveAsPdfOrXps": {
            return wps.PluginStorage.getItem("EnableFileSaveAsPdfOrXps")
        }
        case "FileSaveAsPicture": {
            return wps.PluginStorage.getItem("EnableFileSaveAsPicture")
        }
        default:
            ;
    }
