...
function pOpenFile(doc, params, isOnlineDoc) {
    ...
    if (l_NofityURL) {
        ...
        NotifyToServer(l_NofityURL, {info: "Open succssed"});
    }

    if (doc.ProtectionType === 3) {
        wps.PluginStorage.setItem("EnableCopy", false);
        wps.PluginStorage.setItem("EnableFileSaveAsPdfOrXps", false);
        wps.PluginStorage.setItem("EnableFileSaveAsPicture", false);
        wps.ribbonUI.InvalidateControl("Copy");
        wps.ribbonUI.InvalidateControl("FileSaveAsPdfOrXps");
        wps.ribbonUI.InvalidateControl("FileSaveAsPicture");
    }
...
function pInsertRInedHead(doc, strFile, bookmark) {
    ...
    console.log(wps.WpsApplication().ActiveDocument.Bookmarks)
